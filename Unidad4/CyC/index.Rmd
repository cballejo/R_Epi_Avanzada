...
$$ln(Odds) = \alpha + \sum\beta_ix_i $$
Donde cada $\beta_i$: Incremento en log-odds para una unidad de incremento en xi con todas las otras $x_i$s constantes.
...
En R, el comando nos permite también la comparación
...
La resultados mostrados del objeto de regresión están compuesto por:
...
Este objeto de regresión está compuesto por 30 componentes que pueden accederse a través del nombre del modelo seguido del signo __$__ y el nombre del componente.  
...
__nombre_modelo$iter__: número de iteraciones.

### Gestión de variables dicotómicas

Deciamos que el modelo con enlace *logit* es un modelo de regresión típico, $Y=f(X+E)$, en el que la variable respuesta (variable aleatoria $Y$) es dicotómica o binaria (toma dos valores: 0 y 1), habitualmente sobre si nuestra unidad de análisis tiene una característica (1) o no la tiene (0).
...
Esta evaluación se realiza mediante contrastes de hipótesis cmo el de __Wald__ y la razón de verosimilitud ( __Likelihood Ratio test__ ).
...
Es similar al valor $t$ de la salida en las regresiones lineales y lo utilizamos para determinar la significación de la variable dentro del modelo. La interpretación es flexible y común a las situaciones conocidas, valores de $p$ por debajo de 0,1 permiten que tengamos en cuenta el aporte de dicha variable al modelo.

Por otra parte, la razón de verosimilitud ($G^2$) surge de restar la deviance de un modelo y otro con mas o menos variables predictoras y sirve para determinar si hay diferencia significativa entre incluir o no algunas de estas.
...
__Función lrtest() del paquete lmtest__: Calcula el Likelihood Ratio test (prueba de razón de verosimilitud) entre dos modelos o entre un modelo con variables y el modelo nulo.
...
__Función anova()__: Construye una tabla de análisis de devianza (Analysis of Deviance Table), mediante la forma `anova(nombre_modelo, test = "Chisq")` 
...
Las variables consideradas fueron si al momento de la entrevista caminaban ("Camina" SI-NO), a qué edad habían comenzado a caminar ("Edadmeses"
considerada meses y "Edaddias" medida en días), si la mamá tenía pareja estable ("Parejaest" SI-NO), y si había tomado pecho al menos 6 meses ("Lacto180" SI-NO).
...
Como las categorías de las tres variables convertidas son "SI" y "NO", y el orden automático del lenguaje R es alfabetico el nivel de referencia de para todas es "NO". Confirmemoslo mediante la visualización de los niveles:
...
A continuación iniciamos el proceso manual tipo forward ajustando modelos simples con cada una de las cuatro variables independientes.

__Paso 1__: Se ajustan tantos modelos como posibles variables explicativas tengamos. Dos de las variables (Edaddias y Edadmeses) es la misma expresada en unidades diferentes.
...
```
Se identifica el mayor de estas $G^2$ y se evalúa si es significativo
...
__Paso 2__: Como la variable Lacto180 es significativa, se incorpora como primer variable al modelo y a continuación se generan modelos de dos variables independientes combinando esta variable con cada una de las otras.
...
Se identifica la pareja para la cual la deviance ($-2lnV_{final}$) es menor.
...
lrtest(mod3, mod3.1)
```
__Paso 3__: Como es significativa, se incorpora al modelo y a continuación se agrega la tercer y última variable posible en el modelo (dado que la variable Edadmeses es la misma que Edaddias que está incorporada).
...
summary(modelo_step)
```
El modelo automático es coincidente con el forward manual.
...
Mencionamos diversas formas medir la bondad de ajuste de un modelo de regresión logística. De forma global, ésta puede ser evaluada a través de medidas tipo $R^2$, de probabilidades estimadas o a través de test estadísticos.

Respecto a test estadísticos, la prueba de razón de verosimilitud (Likelihood ratio test), recién vista, es una de las que podemos utilizar basado en la deviance.

Recordemos que podemos aplicar la función `lrtest()` del paquete lmtest o bien la función `anova()`.
...
Entre las pruebas basadas en la agrupación de las probabilidades estimadas bajo el modelo de regresión usaremos el __test de Hosmer y Lemeshow__.
...
extract_eq(mod3.1.1, use_coefs = T, coef_digits = 2)
```


También podemos graficar la __curva ROC__ que es una herramienta más específica de modelos predictivos para comparar diferentes modelos. La curva ROC se obtiene representando, en un cuadrado de lado 1, los valores de 1‐especificidad en el eje de abscisas frente a la sensibilidad en el de las ordenadas para todos los puntos de corte considerados.
...
Encontramos también en el ecosistema del lenguaje R variados paquetes que la implementan. Uno de ellos es __pROC__, mediante su función `roc()`:
...
Podemos visualizar el valor del área bajo la curva (AUC de *Area under the curve*) mediante la misma función e incorporar, por ejemplo, su intervalo de confianza:
...
Una función interesante es `PseudoR2()` perteneciente al paquete __DescTools__ (se debe instalatar previamente). Esta función contiene un grupo de coeficientes desarrollados por autores como *McFadden, Cox y Snell, Efron* y *Nagelkerke*, entre otros.
...
- Si mantenemos constantes las otras variables, la probabilidad de caminar aumentó 20 veces cuando el lactante fue amamantado por 6 meses.

- Si mantenemos constantes las otras variables, la probabilidad de caminar aumentó 1,01 veces por cada día de vida.

- Si mantenemos constantes las otras variables, la probabilidad de caminar aumentó 9,42 veces cuando la madre tenía pareja estable.
...
Por ejemplo, funciones del paquete parameters y see ofrecen la siguiente salida gráfica:
...
Visualisation Toolbox for 'easystats' and Extra Geoms, Themes and Color
Palettes for 'ggplot2'. CRAN. Available from
https://easystats.github.io/see/

Andri Signorell et mult. al. (2021). DescTools: Tools for descriptive
statistics. R package version 0.99.42.

Achim Zeileis, Torsten Hothorn (2002). Diagnostic Checking in Regression
Relationships. R News 2(3), 7-10. URL
https://CRAN.R-project.org/doc/Rnews/

Michael Friendly (2021). vcdExtra: 'vcd' Extensions and Additions. R
...
Daniel D. Sjoberg, Michael Curry, Margie Hannum, Joseph Larmarange,
Karissa Whiting and Emily C. Zabor (2021). gtsummary: Presentation-Ready
Data Summary and Analytic Result Tables. R package version 1.4.2.
