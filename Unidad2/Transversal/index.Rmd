...
Volvamos nuevamente a los estudios transversales: *"Estimar un parámetro poblacional"* podría consistir por ejemplo en *"Estimar la prevalencia de una enfermedad X en la población"*.
...
Para visualizar simulaciones al respecto pueden visitar [Viendo la teoría. Una introducción visual a probabilidad y estadística](https://seeing-theory.brown.edu/frequentist-inference/es.html#section2)

Se puede observar un ejemplo de cálculo de IC para la media basado en la distribución $t$ *de Student* en el siguiente enlace: [Intervalos de confianza con R](https://cballejo.github.io/R_Epi_Avanzada/Unidad2/IC/)
...
Volvamos nuevamente a los estudios de corte transversal: debemos pensarlos como estudios cuyo objetivo es estimar una proporción. Son estudios que intentan responder preguntas como ¿cuál es la prevalencia, proporción o porcentaje del fenómeno en estudio?
...
La variabilidad que estamos dispuestos a aceptar en dicha estimación es lo que llamamos precisión y puede manejarse a través de los intervalos de confianza (es decir, los límites superior e inferior dentro de los cuales se encontrará el verdadero valor del parámetro estimado).
...
Cuando se conoce el tamaño de la población se debe hacer una corrección de la fórmula (ajuste por población finita)
...
Varios son los software estadísticos que calculan tamaños de muestra. Algunos se dedican específicamente a esta tarea mientras que otros la incluyen entre sus opciones. 

Entre los gratuitos podemos mencionar: **EpiDat**, **EpiInfo (StatCalc)**, **OpenEpi**, etc. 

Por su parte el lenguaje R tiene varias funciones incluidas en muchos paquetes que permiten calcular tamaños de muestra.

Dentro de `stats`, perteneciente al R base, encontramos funciones específicas como `power.prop.test()` para comparación de proporciones y `power.t.test()` para test de medias bajo distribuciones $t$ de student.

Otros paquetes con funciones para tamaño de muestra son: `samplingbook`, `samplesize`, `pwr`, `epiR`, etc.

Veamos un ejemplo vinculado con lo explicado anteriormente:

Utilizaremos la función `sample.size.prop()` del paquete **samplingbook** para estimar el tamaño de una muestra para un estudio con el objetivo de estimar la prevalencia de diabetes de la población de una ciudad de 600000 habitantes donde conocemos por estudios previos que pudiera ser cercana a 10,2 %. 

Que tamaño debería tener la muestra (muestreo simple aleatorio)?

Suponiendo que utilizaramos la prevalencia de los estudios previos, el nivel de confianza deseado fuera del 95 % y la precisión aceptada se encuentre en el orden del 2%, hacemos:
...
```
El resultado, aplicada la corrección por población finita, calcula una muestra de 879 personas. 

Como vemos, los argumentos se corresponden con los elementos que integran la fórmula.

- **e**: número positivo que especifica la precisión que es la mitad del ancho del intervalo de confianza 
- **p**: proporción esperada de eventos con dominio entre los valores 0 y 1. El valor predeterminado es `p = 0.5`. 
- **N**: número entero positivo para el tamaño de la población. El valor predeterminado es `N = Inf`, lo que significa que los cálculos se realizan sin *corrección de población finita*. 
- **level**: nivel de confianza para los intervalos de confianza. El valor predeterminado es `level = 0.95`.

Por simplicidad, es habitual considerar infinita a la población objetivo, ya que hacerlo así minimiza los riesgos estadísticos. 

```{r}
sample.size.prop(e = 0.02, P = 0.102, N = Inf, level = 0.95)
```
La muestra, sin corrección por población finita (`N = Inf`), es de 880 personas, solo una mas que la anterior.

Si en cambio, no conociéramos o no confiáramos en el valor de prevalencia de los estudios previos podemos asignar la peor prevalencia esperada (0,5).


```{r}
sample.size.prop(e = 0.02, P = 0.5, N = 600000, level = 0.95)
```
El tamaño muestral es más conservador y aumenta hasta las 2392 unidades de la población.
...
Ya realizó el cálculo del tamaño de la muestra, que deberá ser de 120 alumnos.
...
Por ejemplo, la función `pwr.t.test()` del paquete **pwr**, calcula la potencia para pruebas $t$ de student de medias (para una muestra, dos muestras y muestras pareadas), basado en el tamaño de la muestra, el nivel de confianza y el tamaño de efecto.

Su estructura es:

__pwr.t.test__(n = NULL, d = NULL, sig.level = 0.05, power = NULL, type = c("two.sample", "one.sample", "paired"), alternative = c("two.sided", "less", "greater"))

donde los argumentos son:

__n__:  número de observaciones (para cada grupo, si hubiese 2)

__d__: tamaño de efecto (d de Cohen) - diferencia (estandarizada) entre grupos. 
*Nota:* La $d$ de Cohen representa las desviaciones estándar que separan dos o más grupos, ej: d de Cohen = 0.5 representa que la diferencia entre grupo experimental y muestral es de media desviación estándar. Cohen sugirió (provisoriamente) que 0.2 es un tamaño de efecto pequeño, 0.5 es mediano y 0.8 es grande,

__sig.level__: nivel de significación (probabilidad del error de tipo I)

__power__: potencia del test (1 menos la probabilidad del error tipo II)

__type__: tipo de test ("two.sample", "one.sample", "paired")

__alternative__: palabra que especifica la hipotesis alternativa, debe ser "two.sided" (predeterminado), "greater" or "less".

La función se ejecuta incorporando todos los argumentos obligatorios (`d`,`n`, `power` and `sig.level`) menos el que se quiere calcular. En ese caso se iguala a NULL o se omite. 

Supongamos que queremos conocer el tamaño de la muestra para detectar diferencias en la media de la hemoglobina glicosilada entre dos grupos de pacientes con tratamientos de control de la diabetes distintos. Aceptamos un nivel de efecto convencional de una pequeña desviación ($d$ de Cohen = 0,2), una potencia del 80% y una significación habitual de 0,05.
...
```
El tamaño muestral necesario obtenido es de 393 participantes para cada grupo a comparar.

Esta familia de funciones también permite graficar el tamaño muestral vs la potencia del estudio.

```{r, out.width="60%", fig.align="center"}
grafico1 <- pwr.t.test(d = 0.2, 
           power = 0.8,
           sig.level = 0.05, 
           type = "two.sample", 
           alternative = "two.sided")

plot(grafico1) # ploteamos la salida de grafico1
```

Nos muestra, por ejemplo, que si tomasemos una muestra de 300 individuos por grupo, la potencia del estudio con ese tamaño de efecto y nivel de significación del 0,05  sería aproximadamente de 68 %.
...
**Variable resultado cuantitativa**: En este caso, la comparación de grupos equivale a comparar medias de dichos grupos. Hay una tercera pregunta que debemos responder, y es si dicha variable se *distribuye normalmente* (pulse sobre el enlace para ver material sobre [contrastes de normalidad](https://cballejo.github.io/R_Epi_Avanzada/Unidad2/Supuestos/), porque si no fuera así, debemos recurrir a los contrastes *no paramétricos*. 
...
En el archivo  *“bebe1”* la tabla de datos consiste en una muestra aleatoria de los pesos al nacer de 186 bebés. 
...
b.	En el archivo *“bebe2”*, hay  una muestra aleatoria de 190 bebés cuyas madres manifestaron haber consumido cocaína durante el embarazo. De acuerdo a estos resultados: ¿el consumo de cocaína afecta el peso que tiene un bebé al nacer?
...
En R, lo podemos hacer con la función `z.test()` del paquete **TeachingDemos** que trabaja con valores z de la distribución normal o con la función `t.test()` de R base que utiliza la distribución $t$ *de Student*.

::: {.b--gray .ba .bw2 .ma2 .pa4 .shadow-1}
Para esta etapa donde conectamos elementos teóricos y prácticos mostramos la distribución z aplicada, en cambio verán que a lo largo de curso y en el uso cotidiano de estos métodos se utiliza la distribución $t$ *de Student* - t.test(). 
:::
...
t.test(x = bebe1$peso,
      conf.level = 0.99)
```
Los IC producidos por los test son similares y en la medida que la muestra sea mayor más cercanos serán los valores obtenidos. Fuera de la demostración docente, habitual se utiliza el `t.test()`.
...
```{r, message=F, warning=F, fig.align='center', fig.asp = 0.7, fig.width = 5}
bebe2 <- read_csv2("bebe2.txt")

library(dlookr)

bebe1 %>% 
  plot_normality(peso, col = "firebrick4")

bebe2 %>% 
  plot_normality(peso, col = "olivedrab")

```

```{r}
shapiro.test(x = bebe1$peso)

shapiro.test(x = bebe2$peso)
```
...
```{r}
var.test(bebe1$peso, bebe2$peso)
```
...
Script completo de R para resolver el ejemplo:
...
## Intervalo de confianza para bebe1

## Resumimos la variable peso

describe(bebe1$peso)

##Contraste de normalidad y qqplot

bebe1 %>% 
  plot_normality(peso, col = "firebrick4")

shapiro.test(bebe1$peso)


##Calculo IC con Teaching Demos

z.test(x = bebe1$peso, 
       sd = sd(bebe1$peso), 
       conf.level = 0.99)

##Calculo IC con t.test() - distribución t de student
...
shapiro.test(bebe2$peso)

## Test de homogeneidad de varianzas
var.test(x = bebe1$peso, 
         y = bebe2$peso)

## Test de comparación de medias con varianzas iguales

## contraste bilateral

t.test(x = bebe1$peso,
       y = bebe2$peso,
       alternative = "two.sided",  # valor predeterminado
       mu = 0, # valor predeterminado
       var.equal = TRUE,
       conf.level = 0.95) # valor predeterminado
...
Veamos un ejemplo en R sobre contraste de medias.

Se tiene interés en estudiar la presión arterial en enfermos con un síndrome determinado que toman un nuevo fármaco. Estos enfermos suelen tener una presión arterial media de 165. Con el objetivo de valorar si el nuevo fármaco consigue rebajar la presión arterial de estos enfermos se toma la presión arterial de 15 personas con este síndrome que toman el nuevo fármaco. 

Puede concluirse, con un nivel de confianza del 95%, que los enfermos con este síndrome que toman el nuevo fármaco tienen una presión arterial media menor que 165?
...
unilateral cola izquierda

**Lectura y análisis de supuestos**

```{r, message=F, warning=F}
enfermos <- read_csv2("enfermos.txt") # leemos los datos

enfermos %>%
  summarise(media = mean(V1), # calculamos la media
            desvio = sd(V1)) # calculamos el desvío estandar

 
shapiro.test(enfermos$V1)             # verificamos normalidad
```

Este grupo de enfermos tratados con el nuevo fármaco tienen una media de aproximadamente 148 con un desvío de casi 26. Podemos asegurar que es estadísticamente menor al conocido de 165?

__Prueba de hipótesis__

```{r, message=F}

## ejecutamos el test de contraste para una población
...
Verificamos los supuestos de normalidad la muestra: test shapiro-wilks p-valor = 0,4174.

El p-valor de 0,01158 es significativo (< 0,05) por lo que rechazamos la hipótesis nula de que la presión media de los enfermos que toman el nuevo fármaco es igual 165.  Por lo tanto, concluimos que la media de presión arterial del grupo de pacientes tratados con el nuevo fármaco es significativamente menor del valor objetivo. 
...
Ryu C (2024). _dlookr: Tools for Data Diagnosis, Exploration, Transformation_. R package version 0.6.3, <https://CRAN.R-project.org/package=dlookr>.
