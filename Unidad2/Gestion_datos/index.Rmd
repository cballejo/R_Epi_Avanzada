...
Los argumentos opcionales no son necesarios definirlos si los valores por defecto nos sirven para la tarea buscada.


## Función case_when()

Cuando las condiciones no son simples, es decir, el resultado no es dicotómico, utilizamos la función `case_when()` que es una vectorización de la función anterior (es el equivalente en R del comando CASE WHEN de SQL).
...
Por ejemplo, podríamos tener algun valor perdido (NA) en `var1` y queremos que la variable `grupo_var` etiquete esos valores perdidos como "Sin dato", se puede hacer lo siguiente:
...
Las salidas son de tipo **_carácter_** (chr) y debemos manejar el ordenamiento de las etiquetas como vimos anteriormente, o por medio de factores o comenzando con caracteres ordenados alfabeticamente.
...
Los valores declarados como límites quedan incluídos siempre dentro del intervalo (son cerrados ambos). También podemos utilizar valores reservados como **Inf** o **-Inf** cuando desconocemos con que valor mínimo o máximo nos vamos a encontrar en la variable cuantitativa original.
...
Vemos que en el conteo el formato de la variable ya no es `chr` sino `fctr` y orden de las etiquetas siguen la forma "menor a mayor".
...
Aplicando `case_when()` discretizamos la edad en 4 grupos irregulares, forzando sus etiquetas para lograr el orden adecuado.
...
datos <- datos %>% 
  mutate(grupo3 = case_when(
    ...
    edad > 25 & edad < 65  ~ "3.Adulto Joven",
...
datos <- datos %>% 
  mutate(grupo3 = case_when(
    ...
    between(edad, 26, 64) ~ "3.Adulto Joven",
    between(edad, 65, Inf) ~ "4.Adulto mayor"
  ))
...
Finalmente estas funciones condicionales que estuvimos viendo no se limitan a la tarea de construir agrupamientos de variables cuantitativas sino que sirven para cualquier situación donde a partir de una o más condiciones se produzcan una o más respuestas.