...
parámetros que deben ser escritos **entre paréntesis y separados por comas**. Incluso algunas de ellas que no tienen asociado argumentos necesitan, en su sintaxis, a los paréntesis `()`.
...
negro para objetos (en el theme = Textmate que viene por defecto. En otros temas los colores cambian pero se diferencian entre sí).
...
categóricos y se asocia particularmente con las *variables cualitativas*.
...
casos algunos argumentos de funciones de R exigen que las variables se encuentren
en este formato.
...
Creamos un vector llamado **respuesta** con 7 elementos del tipo caracter donde
se repiten las categorías No y Si.
...
Si queremos comprobar el tipo de objeto en que se convirtió `respuesta``,
encontraremos:
...
objetos, podemos ver que respuesta tiene dos niveles y los valores internos
de los elementos son todos números 1 y 2, donde 1 corresponde a No
y 2 a Si.
...
En estos factores nominales donde no importa el orden, la función `factor()` implementa el orden alfabético para determinar a qué índice numérico pertenece cada categoría. Es claro en
el ejemplo que a *No* le asigna el 1 y a *Si* el 2.
...
Vamos a crear un dataframe llamado `datos` con 4 variables de 5 registros, la primera será un identificador ID numérico entero correlativo, la segunda variable EDAD, la tercera SEXO codificada como V para varón y M para mujer y la tercera TRABAJA de formato
...
En cuanto al sistema de indexación podemos acceder a elementos internos del dataframe de las
siguientes formas:
...
Por ejemplo, en una regresión lineal se utiliza la función base `lm()` y el
