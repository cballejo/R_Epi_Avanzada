...
El sitio oficial **_r-project.org_**^[https://www.r-project.org/] dice que *"R es un entorno de software libre para gráficos y computación estadística. Se compila y se ejecuta en una amplia variedad de plataformas UNIX, Windows y MacOS.”.* 
...
R"**^[https://en.wikipedia.org/wiki/R_(programming_language)], con la intención de desarrollar un programa estadístico inspirado
...
# tenida en cuenta por el intérprete
```
...
El lenguaje evalua las comparaciones que hace el operador relacional
...
Encontrarán en el material que trata sobre RStudio algunas maneras, por
medio de menú, de leer archivos con otros formatos.
...
Observamos que la expresión de la función `read.csv2` se encuentra
...
programa el directorio (carpeta) en que pretendemos trabajar.
...
RStudio (ver en material sobre RStudio) o mediante código.
...
```{r, eval=F}
setwd("C:/CursoAvanzada2021")
```

La línea anterior define como activo al directorio CursoAvanzada2021
...
Con el fin de mostrar algunas de las funciones aplicadas a estadística
descriptiva vamos a utilizar a datos.csv leídos recientemente.
...
sumatoria de todos los valores, en este caso las 74 observaciones.
...
La instalación básica de R y sus paquetes asociados como stats trae
...
Los estadísticos de posición son valores de la variable caracterizados
...
veremos también los cuartiles y deciles muestrales.
...
Uno de los mecanismos de exploración de datos más usuales y útiles
...
potentes para creación de gráficas estadísticas.
...
En este capítulo mencionaremos las funciones más comunes sin entrar con
...
Cuando es necesario analizar la distribución de una variable
...
Mencionaremos aquí algunos argumentos comunes que podemos llegar a
encontrar en las expresiones asociadas a gráficos.
...
En este capítulo mencionaremos algunas consideraciones generales
...
tener en cuenta a la hora de escribirlas:

-   Los argumentos o parametros de una función siempre van entre
...
-   El lenguaje nos ofrece un sistema de ayuda permanente. En Rstudio
    encontramos su panel en el sector abajo a la izquierda de la
    pantalla (en la pestaña Help). También podemos pedir ayuda
    específica mediante la consola. La función `help` o el símbolo `?`
    ...
    subtitulo **Usage** y los argumentos en **Arguments**
